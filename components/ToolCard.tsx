...
export default function ToolCard({ tool }: ToolCardProps) {
  ...
  return (
    <a
      ...
      className="group flex flex-col rounded-xl border border-zinc-200 bg-white p-5 shadow-sm transition-all hover:border-zinc-300 hover:shadow-md dark:border-zinc-700 dark:bg-zinc-900 dark:hover:border-zinc-600"
    >
      {/* Header row */}
      <div className="flex items-start justify-between gap-2">
        <h3 className="line-clamp-1 text-base font-semibold text-zinc-900 group-hover:text-blue-600 dark:text-zinc-50 dark:group-hover:text-blue-400">
          {tool.name}
        </h3>
      ...
      {/* Footer row: category badge + stars */}
      <div className="mt-3 flex items-center justify-between gap-2">
        ...
        {/* Stars */}
        <div className="flex items-center gap-1 text-sm text-zinc-500 dark:text-zinc-400" title={`${tool.stars} stars`}>
      ...
      {tool.tags.length > 0 && (
        <div className="mt-2 flex flex-wrap gap-1">
          {tool.tags.map((tag) => (
            <span key={tag} className="rounded bg-blue-50 px-1.5 py-0.5 text-[10px] font-medium text-blue-700 dark:bg-blue-900/30 dark:text-blue-300">
              {tag}
            </span>
          ))}
        </div>
